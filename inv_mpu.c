...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        if (enable)
            ...
            tmp = 0x00;
        if (i2c_write(&e, st.reg->int_enable, 1, &tmp))
            return -1;
...
int mpu_init(void)
{
    ...
    if (i2c_write(&e, st.reg->pwr_mgmt_1, 1, &(data[0])))
        return -1;
    usleep(100000);
    ...
    if (i2c_read(&e, st.reg->accel_offs, 6, data))
        return -1;
    ...
    if (rev) {
        ...
        if (i2c_read(&e, st.reg->prod_id, 1, &(data[0])))
            return -1;
        rev = data[0] & 0x0F;
        if (!rev) {
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    if (i2c_write(&e, st.reg->accel_cfg2, 1, data))
        return -1;
#endif
...
int mpu_lp_accel_mode(unsigned char rate)
{
    ...
    if (!rate) {
        ...
        tmp[0] = 0;
        tmp[1] = BIT_STBY_XYZG;
        if (i2c_write(&e, st.reg->pwr_mgmt_1, 2, tmp))
            return -1;
        st.chip_cfg.lp_accel_mode = 0;
        return 0;
    }
    ...
    if (rate == 1)
        ...
        tmp[0] = INV_LPA_640HZ;
    if (i2c_write(&e, st.reg->lp_accel_odr, 1, tmp))
        return -1;
    tmp[0] = BIT_LPA_CYCLE;
    if (i2c_write(&e, st.reg->pwr_mgmt_1, 1, tmp))
        return -1;
#endif
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(&e, st.reg->raw_gyro, 6, tmp))
        return -1;
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(&e, st.reg->raw_accel, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(&e, st.reg->temp, 2, tmp))
        return -1;
    raw = (tmp[0] << 8) | tmp[1];
    if (timestamp)
        get_ms(timestamp);
...
int mpu_reset_fifo(void)
{
    ...
    data = 0;
    if (i2c_write(&e, st.reg->int_enable, 1, &data))
        return -1;
    if (i2c_write(&e, st.reg->fifo_en, 1, &data))
        return -1;
    if (i2c_write(&e, st.reg->user_ctrl, 1, &data))
        return -1;
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (i2c_write(&e, st.reg->int_enable, 1, &data))
            return -1;
        if (i2c_write(&e, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
            return -1;
    }
    return 0;
}
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    if (i2c_write(&e, st.reg->gyro_cfg, 1, &data))
        return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (i2c_write(&e, st.reg->accel_cfg, 1, &data))
        return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == data)
        return 0;
    if (i2c_write(&e, st.reg->lpf, 1, &data))
        return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        data = 1000 / rate - 1;
        if (i2c_write(&e, st.reg->rate_div, 1, &data))
            return -1;
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
    ...
    if (i2c_write(&e, st.reg->s4_ctrl, 1, &div))
        return -1;
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors & INV_XYZ_GYRO)
        ...
        data = BIT_SLEEP;
    if (i2c_write(&e, st.reg->pwr_mgmt_1, 1, &data)) {
    ...
    if (!(sensors & INV_XYZ_ACCEL))
        data |= BIT_STBY_XYZA;
    if (i2c_write(&e, st.reg->pwr_mgmt_2, 1, &data)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
    ...
    if (sensors & INV_XYZ_COMPASS)
        ...
        mpu_set_bypass(0);
#else
    if (i2c_read(&e, st.reg->user_ctrl, 1, &user_ctrl))
        return -1;
    ...
    if (st.chip_cfg.dmp_on)
        ...
        user_ctrl &= ~BIT_DMP_EN;
    if (i2c_write(&e, st.reg->s1_do, 1, &data))
        return -1;
    /* Enable/disable I2C master mode. */
    if (i2c_write(&e, st.reg->user_ctrl, 1, &user_ctrl))
        return -1;
#endif
#endif
...
int mpu_get_int_status(short *status)
{
    ...
    if (!st.chip_cfg.sensors)
        return -1;
    if (i2c_read(&e, st.reg->dmp_int_status, 2, tmp))
        return -1;
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (i2c_read(&e, st.reg->fifo_count_h, 2, data))
        return -1;
...
//    log_i("FIFO count: %hd\n", fifo_count);
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (i2c_read(&e, st.reg->int_status, 1, data))
            return -1;
    ...
    if (i2c_read(&e, st.reg->fifo_r_w, packet_size, data))
        return -1;
    ...
    unsigned char *more)
{
    ...
    if (i2c_read(&e, st.reg->fifo_count_h, 2, tmp))
        return -1;
	DEBUG_printf("mpu_read_fifo_stream 000000000000000\n");	
    fifo_count = (tmp[0] << 8) | tmp[1];
    if (fifo_count < length) {
        more[0] = 0;
        return -1;
    }
	DEBUG_printf("mpu_read_fifo_stream 111111111111111111\n");
	///*
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        // FIFO is 50% full, better check overflow bit. 
        if (i2c_read(&e, st.reg->int_status, 1, tmp))
            return -1;
        if (tmp[0] & BIT_FIFO_OVERFLOW) {
            mpu_reset_fifo();
			DEBUG_printf("mpu_read_fifo_stream 22222222222222222222222\n");
            return -2;
        }
    }
	//*/
	DEBUG_printf("mpu_read_fifo_stream 3333333333333333333\n");
    if (i2c_read(&e, st.reg->fifo_r_w, length, data))
        return -1;
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (i2c_read(&e, st.reg->user_ctrl, 1, &tmp))
            return -1;
        tmp &= ~BIT_AUX_IF_EN;
        if (i2c_write(&e, st.reg->user_ctrl, 1, &tmp))
            return -1;
        usleep(3000);
        tmp = BIT_BYPASS_EN;
        ...
        if (i2c_write(&e, st.reg->int_pin_cfg, 1, &tmp))
            return -1;
    } else {
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    data[0] = 0x01;
    data[1] = 0;
    if (i2c_write(&e, st.reg->pwr_mgmt_1, 2, data))
        return -1;
    usleep(200000);
    data[0] = 0;
    if (i2c_write(&e, st.reg->int_enable, 1, data))
    ...
    if (i2c_write(&e, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    if (i2c_write(&e, st.reg->i2c_mst, 1, data))
        return -1;
    if (i2c_write(&e, st.reg->user_ctrl, 1, data))
        return -1;
...
*////////////////////		
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (i2c_read(&e, st.reg->fifo_r_w, MAX_PACKET_LENGTH, data))
            return -1;
        ...
        unsigned char *data)
{
    ...
    if (i2c_write(&e, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2c_write(&e, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
        ...
        unsigned char *data)
{
    ...
    if (i2c_write(&e, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2c_read(&e, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
...
#define LOAD_CHUNK  (16)
	...
	DEBUG_printf("3333333333\n");
    /* Set program start address. */
    tmp[0] = start_addr >> 8;
    tmp[1] = start_addr & 0xFF;
    if (i2c_write(&e, st.reg->prgm_start_h, 2, tmp))
        return -1;
...
//    if (i2c_write(&e, st.reg->s0_addr, 1, data))
//        return -1;
//
//    /* Compass reads start at this register. */
//    data[0] = AKM_REG_ST1;
//    if (i2c_write(&e, st.reg->s0_reg, 1, data))
//        return -1;
//
//    /* Enable slave 0, 8-byte reads. */
//    data[0] = BIT_SLAVE_EN | 8;
//    if (i2c_write(&e, st.reg->s0_ctrl, 1, data))
//        return -1;
//
//    /* Slave 1 changes AKM measurement mode. */
//    data[0] = st.chip_cfg.compass_addr;
//    if (i2c_write(&e, st.reg->s1_addr, 1, data))
//        return -1;
//
//    /* AKM measurement mode register. */
//    data[0] = AKM_REG_CNTL;
//    if (i2c_write(&e, st.reg->s1_reg, 1, data))
//        return -1;
//
//    /* Enable slave 1, 1-byte writes. */
//    data[0] = BIT_SLAVE_EN | 1;
//    if (i2c_write(&e, st.reg->s1_ctrl, 1, data))
//        return -1;
//
//    /* Set slave 1 data. */
...
//    data[0] = 0x03;
//    if (i2c_write(&e, st.reg->i2c_delay_ctrl, 1, data))
//        return -1;
//
//#ifdef MPU9150
//    /* For the MPU9150, the auxiliary I2C bus needs to be set to VDD. */
//    data[0] = BIT_I2C_MST_VDDIO;
//    if (i2c_write(&e, st.reg->yg_offs_tc, 1, data))
//        return -1;
//#endif
//
//    return 0;
...
int mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+8))
        return -1;
#else
    if (i2c_read(&e, st.reg->raw_compass, 8, tmp))
        return -1;
#endif
    ...
    unsigned char lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        data[0] = thresh_hw;
        data[1] = time;
        if (i2c_write(&e, st.reg->motion_thr, 2, data))
            goto lp_int_restore;
        ...
        if (i2c_write(&e, st.reg->user_ctrl, 3, data))
            goto lp_int_restore;

        /* Set motion threshold. */
        data[0] = thresh_hw;
        if (i2c_write(&e, st.reg->motion_thr, 1, data))
            goto lp_int_restore;
        ...
        if (i2c_write(&e, st.reg->lp_accel_odr, 1, data))
            goto lp_int_restore;

        /* Enable motion interrupt (MPU6500 version). */
        data[0] = BITS_WOM_EN;
        if (i2c_write(&e, st.reg->accel_intel, 1, data))
            goto lp_int_restore;
