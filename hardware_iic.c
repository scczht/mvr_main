...
__u8 my_read_byte(struct eeprom* e, __u8 mem_addr)
{
	...
	r = i2c_write_1b(e, mem_addr);
	if (r < 0)
	{
		printf("Read set addresseserror\n");
		return 99;
	}
...
{	int i;
	...
	for(i=0;i<len;i++)
	{
		r = i2c_smbus_write_byte(e->fd, reg++);
		if(r < 0)
		{
			fprintf(stderr, "Error i2c_write_1b: %s\n", strerror(errno));
			return -1;
		...
		usleep(10);	
		r = i2c_smbus_write_byte(e->fd, *(data++));
		if(r < 0)
		{
			fprintf(stderr, "Error i2c_write_1b: %s\n", strerror(errno));
			return -1;
...
int i2c_read11(struct eeprom *e, __u8 reg, __u8 len, __u8 *buf)
{
	...
	int i;
	ioctl(e->fd, BLKFLSBUF); // clear kernel read buffer
	r=i2c_write_1b(e,reg);
	if(r < 0)
	{
		fprintf(stderr, "IN  read Error i2c_write_1b: %s\n", strerror(errno));
			return -1;
	}
	for(i=0;i<len;i++)
	{
		*(buf++)=i2c_smbus_read_byte(e->fd);  
	}
	return 0;
}


static inline __s32 my_i2c_smbus_write_i2c_block_data(int file, __u8 command,
                                               __u8 length, __u8 *values)
{
	union i2c_smbus_data data;
	int i;
...
{	
	...
	my_i2c_smbus_write_i2c_block_data(e->fd,reg, len,data);
	usleep(10);
	return 0;
}

/* Returns the number of read bytes */
static inline __s32 my_i2c_smbus_read_i2c_block_data(int file, __u8 command,__u8 len, __u8 *values)
{
	union i2c_smbus_data data;
	int i;
	data.block[0]=len;
...
{
	 my_i2c_smbus_read_i2c_block_data(e->fd,reg,len,buf);
	//i2c_smbus_read_i2c_block_data(e->fd,reg, len, &buf);
	return 0;
}
...
void do_usage_if(int b, int line)
{
	const static char *eeprog_usage =
		"MPU6050 DMP !\n"
		"Base on MPU6050_LIB  FriendlyARM_24c08  and so on\n"
		"Thanks to source contributors\n";
		"QQ: 891948292\n";
	if(!b)
		return;
	fprintf(stderr, "%s\n[line %d]\n", eeprog_usage, line);
